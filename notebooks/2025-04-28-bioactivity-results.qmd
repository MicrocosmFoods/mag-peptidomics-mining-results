...
3.  \~11,000 bacterial metagenome-assembled genomes (MAGs) and reference genomes. We refer to this as the "MAG" dataset as it is primarily comprised of genomes from fermented food metagenomic surveys
...
proteomics_bioactivity_results <- read_tsv("../results/2025-02-20-proteomics-bioactivity-results/ff_peptidomics_peptides_predictions.tsv") %>% 
...
write_tsv(proteomics_bioactivity_metadata, "../results/2025-02-20-proteomics-bioactivity-results/ff_peptidomics_peptides_metadata.tsv")

proteomics_bioactivity_df <- prepare_bioactivity_analysis(proteomics_bioactivity_metadata, category_column = "fermented_food")

filtered_proteomics_bioactivity_metadata <- proteomics_bioactivity_df[["filtered_bioactivity_df"]]

write_tsv(filtered_proteomics_bioactivity_metadata, "../results/cleaned_results/2025-08-05-filtered-proteomics-bioactivity.tsv")

# bacdive results and metadata
bacdive_bioactivity_results <- read_tsv("../results/2025-02-24-bacdive-results/2025-02-24-bacdive-peptides-predictions.tsv") %>% 
...
bacdive_metadata <- read_tsv("../metadata/2024-12-06-bacdive-accessions-curated-metadata.tsv") %>% 
    mutate(ingredient_group = ifelse(ingredient_group == "grain", "grains", ingredient_group))

bacdive_bioactivity_metadata <- left_join(bacdive_bioactivity_results, bacdive_metadata) %>%
  filter(!is.na(ingredient_group)) %>% 
...
mag_bioactivity_info_metadata <- process_peptide_bioactivity_info(mag_bioactivity_metadata, mag_deeppeptide_results, mag_smorfinder_results, mag_ripp_results) %>% 
  select(genome_name, peptide_id, peptide_type, everything())

write_tsv(mag_bioactivity_info_metadata, "../results/2025-04-24-mag-results/2025-08-05-mag-bioactivity-info.tsv")

mag_ff_bioactivity_df <- prepare_bioactivity_analysis(mag_bioactivity_info_metadata, category_column="ingredient_group")

filtered_ff_mag_bioactivity <- mag_ff_bioactivity_df[["filtered_bioactivity_df"]]

write_tsv(filtered_ff_mag_bioactivity, "../results/cleaned_results/2025-08-08-filtered-mag-bioactivity-df.tsv")
...
mag_summary_stats <- generate_bioactivity_summary(mag_ff_bioactivity_df)
...
The first plot below are bioactivity counts for BacDive genomes and the second from the MAG dataset.

```{r bacdive_mag_bioactivity_labels, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10}

plot_bioactivity_distribution(bacdive_bioactivity_df[["filtered_bioactivity_df"]])
plot_bioactivity_distribution(mag_ff_bioactivity_df[["filtered_bioactivity_df"]])
...
proteomics_peptipedia_hits <- process_peptipedia_hits(proteomics_bioactivity_metadata, peptipedia_metadata, category_column = "fermented_food")
